...
class PistolTheWeapon(Weapon):
    ...
    def shoot(self, weapon_target_x, weapon_target_y):
        ...
        if not shooting_allowed:
            return
        bullet = Bullet(bullet_originator=self.player, bullet_x=self.player.player_x, bullet_y=self.player.player_y, bullet_target_x=weapon_target_x, bullet_target_y=weapon_target_y, bullet_speed=self.weapon_bullet_speed, bullet_color=self.player.player_color, bullet_damage=10)
